/**
 * 동적 서비스용 파라미터화 SQL 빌더
 * 모든 VALUE는 ? 바인딩, 테이블/컬럼명은 백틱 이스케이핑
 */
 ...
 * @param {string} tableName - 백틱 포함 테이블명
 * @param {string} templateType - basic | company | restaurant
 * @param {Array} customFields - service_field_configs 레코드 배열
 * @returns {string} CREATE TABLE SQL
 */
function buildCreateEntitiesSQL(tableName, templateType, customFields = []) {
    const columns = [
 ...
 */
function buildCreateBoardsSQL(tableName) {
    return `CREATE TABLE ${tableName} (
 ...
 */
function buildCreateCommentsSQL(tableName) {
    return `CREATE TABLE ${tableName} (
 ...
 */
function buildCreateRequestsSQL(tableName) {
    return `CREATE TABLE ${tableName} (
 ...
 * SELECT 쿼리 빌더 (엔티티 목록)
 * @param {string} tableName - 백틱 포함 테이블명
 * @param {Object} options - { page, limit, where, orderBy, columns }
...
function buildSelectSQL(tableName, options = {}) {
    ...
    const colStr = columns.map(c => c === '*' ? '*' : `\`${c}\``).join(', ');
 ...
 * COUNT 쿼리 빌더
 */
function buildCountSQL(tableName, where = {}) {
 ...
 * @param {Object} data - { column: value } 객체
 * @param {Array} allowedColumns - 허용된 컬럼 화이트리스트
 * @returns {{ sql: string, params: Array }}
 */
function buildInsertSQL(tableName, data, allowedColumns) {
    const cols = [];
    const placeholders = [];
    const params = [];
 ...
 * @param {string} idColumn - PK 컬럼명
 * @param {*} idValue - PK 값
 * @param {Array} allowedColumns - 허용된 컬럼 화이트리스트
 * @returns {{ sql: string, params: Array }}
 */
function buildUpdateSQL(tableName, data, idColumn, idValue, allowedColumns) {
    const setClauses = [];
    const params = [];
 ...
 */
function buildSoftDeleteSQL(tableName, idColumn, idValue) {
    return {
