const { CompComment, sequelize } = require('../model/index');
...
exports.getComments = async (boardIdx) => {
    return await CompComment.findAll({
...
exports.insertComment = async (commentData) => {
    ...
    try {
        const now = new Date();
        // 댓글 생성
        const comment = await CompComment.create({
        ...
        logger.debug(`[insertComment] CompComment created. CommentId: ${comment.commentIdx}`);
...
exports.modifyComment = async (commentData) => {
    ...
    try {
        // 비밀번호 검증
        const comment = await CompComment.findOne({
