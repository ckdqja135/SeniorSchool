...
exports.getOutsourceBoards = async (outsourceIdx, searchParams = {}) => {
    try {
        ...
        logger.info(`[getOutsourceBoards] Found ${boards.length} boards for outsource ${outsourceIdx}`);
        return boards;
    } catch (error) {
...
exports.getOutsourceBoardDetail = async (boardIdx) => {
    try {
        ...
        logger.info(`[getOutsourceBoardDetail] Board detail retrieved. BoardIdx: ${boardIdx}`);
        return board;
    } catch (error) {
