 ...
 *
 * Backend/service/restaurantCrawlerService.js의 1:1 포팅.
 *  - axios / 외부 API 호출부는 원본 그대로 유지 (Gemini/kakao/naver/google/siksin).
 *  - Sequelize 모델 접근(RestaurantInfo.findAll/bulkCreate/update)은 PrismaService로 치환한다.
 ...
 */
function convertNaverCoords(mapx: any, mapy: any): { lat: number; lng: number } {
...
export class RestaurantCrawlerService {
    ...
    async fetchFromNaver({ query = '맛집', count = 50 }: any): Promise<any[]> {
    ...
    // ─── 3) Google Places API ────────────────────────────────────
    async fetchFromGoogle({ query = 'restaurant in Seoul', lat = 37.5665, lng = 126.9780, radius = 5000, count = 50 }: any): Promise<any[]> {
     ...
     */
    private async getSiksinToken(): Promise<string | null> {
        try {
     ...
     */
    private async fetchSiksinMenu(pid: any, token: any): Promise<any[] | null> {
        try {
     ...
     */
    private async getSiksinAreaIds(token: any, region: string): Promise<any[]> {
    ...
    async fetchFromSiksin({ region = '서울', count = 50, query = '' }: any): Promise<any[]> {
        const token = await this.getSiksinToken();
        if (!token) {
            logger.warn('[Crawler:Siksin] 토큰 없이 HTML 방식으로 폴백');
            return this.fetchFromSiksinFallback({ region, count, query });
        }
        ...
        const areaIds = (await this.getSiksinAreaIds(token, region)).sort(() => Math.random() - 0.5);
        if (areaIds.length === 0) {
                ...
                try {
                    ...
                    for (const store of stores) {
                        ...
                        if ((!menu || menu.length === 0) && pid) {
                            menu = await this.fetchSiksinMenu(pid, token);
                        }
     ...
     */
    private async fetchFromSiksinFallback({ region = '서울', count = 50, query = '' }: any): Promise<any[]> {
        const results: any[] = [];
        const seen = new Set();
        let page = 1;
    ...
    private async geocodeByKakao(address: string): Promise<{ lat: number; lng: number } | null> {
        const key = process.env.KAKAO_REST_API_KEY;
    ...
    // (구 Sequelize Op.between → Prisma gte/lte, Op.or → OR)
    private async findExisting(restaurant: any): Promise<any> {
        return await this.prisma.restaurantInfo.findFirst({
            where: {
                restaurantStatus: 1,
                OR: [
                    {
                    ...
                    ...(restaurant.restaurantLatX && restaurant.restaurantLatY ? [{
                        restaurantName: restaurant.restaurantName,
                        restaurantLatX: {
                            gte: restaurant.restaurantLatX - 0.0005,
                            lte: restaurant.restaurantLatX + 0.0005,
                        },
                        restaurantLatY: {
                            gte: restaurant.restaurantLatY - 0.0006,
                            lte: restaurant.restaurantLatY + 0.0006,
                        },
                    }] : []),
                ],
            } as any,
     ...
     */
    async crawlRestaurants(options: any = {}): Promise<any> {
        const {
        ...
        const existingRecords = await this.prisma.restaurantInfo.findMany({
            where: {
        ...
        for (const item of newResults) {
            if ((!item.restaurantLatX || !item.restaurantLatY) && item.restaurantAddr) {
                const coords = await this.geocodeByKakao(item.restaurantAddr);
                if (coords) {
        ...
        if (saveToDB && bulkData.length > 0) {
            try {
                ...
                // 구 Sequelize bulkCreate(updateOnDuplicate) 재현:
                //  INSERT ... ON DUPLICATE KEY UPDATE (restaurantURL, restaurantType, restaurantLatX, restaurantLatY)
                //  → uq_restaurant_name_addr 유니크 기준 per-row upsert 를 트랜잭션으로 원자적 실행.
                const result = await this.prisma.$transaction(
                    bulkData.map((item) => this.prisma.restaurantInfo.upsert({
                        where: {
                        ...
                        create: {
                            ...
                            restaurantLotAddr: item.restaurantLotAddr,
                            restaurantAddr: item.restaurantAddr,
                            restaurantMapIMG: item.restaurantMapIMG,
                            restaurantImage: item.restaurantImage,
                            // 구 Sequelize restaurantMenu setter 재현: 배열→JSON.stringify, null→null
                            restaurantMenu: item.restaurantMenu ? JSON.stringify(item.restaurantMenu) : null,
                            restaurantStatus: item.restaurantStatus,
                            restaurantViewCount: item.restaurantViewCount,
                        },
                    }))
                );
    ...
    async enrichFromSiksin(restaurantName: string): Promise<any> {
        const token = await this.getSiksinToken();
