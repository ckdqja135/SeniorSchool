// Backend/service/companyCrawlerService.js 의 1:1 포팅.
// 회사 데이터 통합 크롤러 서비스
...
async function fetchFromKakao({ query = '회사', region = '서울', count = 50, lat, lng, radius = 20000 }: any): Promise<any[]> {
...
async function fetchFromNaver({ query = '회사', region = '서울', count = 50 }: any): Promise<any[]> {
...
async function fetchFromPublicData({ query = '회사', region = '서울', count = 50 }: any): Promise<any[]> {
...
async function geocodeByKakao(address: any): Promise<{ lat: number; lng: number } | null> {
    const key = process.env.KAKAO_REST_API_KEY;
...
async function findHomepageByNaver(companyName: any): Promise<any> {
    const clientId = process.env.NAVER_CLIENT_ID;
    const clientSecret = process.env.NAVER_CLIENT_SECRET;
...
async function findLocationByNaver(companyName: any): Promise<any> {
    const clientId = process.env.NAVER_CLIENT_ID;
    const clientSecret = process.env.NAVER_CLIENT_SECRET;
    if (!clientId || !clientSecret) return null;
    ...
    try {
        ...
        const normalize = (s: string) => s.replace(/[\s·\-()（）주식회사㈜]/g, '').toLowerCase();
 ...
 */
async function enrichFromNaver(companyName: any): Promise<any> {
...
// ─── 소스별 상태 확인 ────────────────────────────────────────
function getAvailableSources(): any[] {
    const sources: any[] = [];
...
export class CompanyCrawlerService {
    ...
    fetchFromKakao(opts: any): Promise<any[]> {
        return fetchFromKakao(opts);
    }

    fetchFromNaver(opts: any): Promise<any[]> {
        return fetchFromNaver(opts);
    }

    fetchFromPublicData(opts: any): Promise<any[]> {
        return fetchFromPublicData(opts);
    }

    enrichFromNaver(companyName: any): Promise<any> {
        return enrichFromNaver(companyName);
    }

    // ─── 통합 크롤링 실행 ────────────────────────────────────────
    async crawlCompanies(options: any = {}): Promise<any> {
        const {
            sources = ['kakao', 'naver'],
            query = '회사',
            region = '서울',
        ...
        for (const item of newResults) {
            const classified = await this.conglomerate.classifyCompType(item.compName);
            if (classified === '대기업') {
                item.compType = '대기업';
        ...
        if (saveToDB && bulkData.length > 0) {
            try {
                // 원본은 Sequelize bulkCreate({ updateOnDuplicate: [...] }). tb_comp_info 는 PK(compIdx, auto)
                // 외 UNIQUE 키가 없어 ON DUPLICATE KEY UPDATE 가 발동하지 않으므로 전건 INSERT 와 동일.
                // Prisma createMany 로 동일 동작을 재현한다.
                const result = await this.prisma.compInfo.createMany({ data: bulkData as any });
                stats.saved = result.count;
                for (const d of bulkData) savedList.push(d.compName);
            } catch (err: any) {
