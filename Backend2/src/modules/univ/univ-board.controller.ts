// Backend/routes/univBoard.router.js + controller/univBoardController.js의 포팅.
...
@Controller('univ/board')
export class UnivBoardController {
    constructor(private readonly univBoardService: UnivBoardService) {}
    ...
    async getBoards(@Req() req: Request, @Res() res: Response) {
        try {
            ...
            const boards = await this.univBoardService.getBoards(univIdx, searchParams);
    ...
    async getBoardDetail(@Req() req: Request, @Res() res: Response) {
        try {
            ...
            const detailBoard = await this.univBoardService.getBoardDetail(boardIdx);
    ...
    async insertBoard(@Req() req: Request, @Res() res: Response) {
        try {
            ...
            const result = await this.univBoardService.insertBoard(boardData);
    ...
    async correctBoard(@Req() req: Request, @Res() res: Response) {
        try {
            const boardData = req.body;
            const result = await this.univBoardService.correctBoard(boardData);
            res.status(200).json({ success: true, message: result });
        } catch (error) {
            logger.error(error);
            res.status(500).json({ error: 'Internal Server Error' });
    ...
    async deleteBoard(@Req() req: Request, @Res() res: Response) {
        try {
            const boardData = req.body;
            const result = await this.univBoardService.deleteBoard(boardData);
            res.status(200).json({ success: true, message: result });
        } catch (error) {
            logger.error(error);
            res.status(500).json({ error: 'Internal Server Error' });
    ...
    async toggleBoardLike(@Req() req: Request, @Res() res: Response) {
        try {
            ...
            const result = await this.univBoardService.toggleBoardLike(boardIdx, isLiked);
    ...
    async getBoardLike(@Req() req: Request, @Res() res: Response) {
        try {
            ...
            const likeCount = await this.univBoardService.getBoardLike(boardId);
    ...
    async getRecentBoardsWithUnivInfo(@Req() req: Request, @Res() res: Response) {
        try {
            const result = await this.univBoardService.getRecentBoardsWithUnivInfo();
    ...
    async getTopViewedBoardsByUniversity(@Req() req: Request, @Res() res: Response) {
        try {
            const result = await this.univBoardService.getTopViewedBoardsByUniversity();
